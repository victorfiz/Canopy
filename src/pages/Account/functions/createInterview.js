...
export const createInterview = async ({ interview_template_id, uid }) => {
    ...
    try {
        ...
        console.log(interview_doc.id)
        console.log(pre_interview_skip_resume)

        const content  =interview_template_doc.data().pre_interview_name + ((!pre_interview_skip_resume||(pre_interview_skip_resume.trim()==="false")) ? resume_text_content:"" ) + `Include the string "[SECTION:1] at the start of each message to signal that you are on section 1.`
        console.log(content)
